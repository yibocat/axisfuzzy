...
def run_nested_pipeline_example():
    ...
    # 根据我们对 _pipeline.py 的修改，如果子管道有多个末端输出，
    ...
    # 根据之前对 _pipeline.py 的修改，如果子管道有多个末端输出，
    ...
    for step_id, result_data in intermediate_states.items():
        # get_step_info() now returns a StepMetadata object
        step_meta = main_pipeline.get_step_info(step_id)

        # Access attributes using dot notation, not dictionary keys
        print(f"\nStep '{step_meta.display_name}' (ID: {step_id[:8]}...):")

        # For nested pipeline steps, its result is the sub-pipeline's final output
        if step_meta.display_name == preprocessing_sub_pipeline.name:
